...
function QuickFillPopup({ exercise, onConfirm, onCancel }) {
  const [values, setValues] = useState({ numSets: 4, reps: 10, weight: 50, restTime: 60 });
  ...
  return (
    <div className="fixed inset-0 bg-black/80 flex items-end justify-center z-50 p-4">
      <div className="bg-gray-900 border border-gray-700 rounded-2xl w-full max-w-sm p-6 space-y-5">
        ...
        {[
          { label: 'Nombre de séries', field: 'numSets', suffix: 'séries' },
          { label: 'Reps par défaut', field: 'reps', suffix: 'reps' },
          { label: 'Poids par défaut', field: 'weight', suffix: 'kg' },
          { label: 'Repos par défaut', field: 'restTime', suffix: 'sec' },
        ].map(({ label, field, suffix }) => (
          <div key={field} className="flex items-center justify-between">
            <span className="text-sm text-gray-300">{label}</span>
            <div className="flex items-center gap-2">
              <button
                onClick={() => handleChange(field, values[field] - (field === 'restTime' ? 15 : 1))}
...
// ─── Set Editor Row ──────────────────────────────────────────────────────────
function SetRow({ set, isEditing, onToggleEdit, onChange, onDelete }) {
  return (
    <div className="flex items-center gap-2 bg-gray-800/60 rounded-lg px-3 py-2">
      ...
      {isEditing ? (
        ...
        <>
          <span className="text-sm text-white flex-1">{set.reps} reps · {set.weight}kg · {set.restTime}s</span>
...
export default function CustomProgramCreator({ customExercises = [], onSave, onBack }) {
  ...
  const handleQuickFillConfirm = (sets) => {
